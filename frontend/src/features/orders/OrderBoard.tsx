...
const OrderBoard: React.FC<Props> = ({ orders, onAdvance, visibleStatuses }) => {
  return (
    <div style={{ flex: 1, padding: '1rem', overflowY: 'auto' }}>
      ...
      {(visibleStatuses ?? statusFlow).map(status => (
        <div key={status} style={{ marginBottom: '1rem' }}>
          ...
          {orders.filter(o => o.status === status).length === 0 && <p>No orders</p>}
          {orders
              ...
              return (
                ...
                >
                  ...
                  <ul style={{ margin: '0.25rem 0 0.5rem 1rem', padding: 0 }}>
                    {o.items.map(it => {
                      const product = DataService.getProducts().find(p => p.id === it.productId);

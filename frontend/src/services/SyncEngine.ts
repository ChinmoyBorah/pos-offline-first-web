...
class SyncEngine {
  ...
  private API_BASE = (import.meta as any).env?.VITE_API_URL || 'http://localhost:5000';
  ...
  private set lastSyncAt(val: number) {
    localStorage.setItem(SyncEngine.META_KEY, JSON.stringify({ lastSyncAt: val }));
  }
  ...
  private async tick() {
    ...
    try {
      const res = await fetch(`${this.API_BASE}/sync`, {
        ...
        body: JSON.stringify({ changes: queue, lastSyncAt: this.lastSyncAt }),
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();

      // remove accepted ids from queue
      const remaining = queue.filter((c: any) => !data.acceptedIds.includes(c.id));
